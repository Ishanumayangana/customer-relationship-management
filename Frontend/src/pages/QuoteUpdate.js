...
function QuoteUpdate() {
    ...
    const handleSubmit = () => {
        const url = "http://localhost:8000/crm/update-Quote/" + id;
    ...
    return (
        <div>
            <form onSubmit={handleSubmit}>
                <Quoform getState={getState} btnName="Update Student" clientValue={obj.client} noValue={obj.number} yearValue={obj.year} statValue={obj.stat} noteValue={obj.note} datValue={obj.dat}  expireValue={obj.expire} itemValue={obj.item} descValue={obj.desc}  qtyValue={obj.qty} priceValue={obj.price} totalValue={obj.total}/>
            </form>
        </div>
    )
}
